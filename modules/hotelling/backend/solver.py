...
Moved from ``solvers/hotelling.py`` in the hotelling/nash feature order
(v1 O11 / v2 O15, ``docs/feature-modules-plan.md``), with the
competitive-fallback cap rules INJECTED (plan §2 "cap_rules replaces
MSRState import"): the engine-bound entry point
(``ets.engine.wiring.solve_hotelling_path``) wires
``default_cap_rules(m0, "hotelling")`` — the F2-frozen fallback wiring (a
per-year-gated MSR iff ``m0.msr_enabled``, never a CCR; R8/R9). The PRIMARY
Hotelling path applies no rules (result rows hardcode ``msr_* = 0.0`` — F2).
...
def _competitive_fallback(ordered_markets, cap_rules: Sequence[CapRule] = ()) -> list[dict]:
    # cap_rules: injected by the engine-bound entry point via
    # default_cap_rules(m0, "hotelling") — the retired legacy construction
    # here was MSRState() iff m0.msr_enabled, i.e. exactly [MSRCapRule()]
    # under the legacy-kwarg translation (never a CCR; F2/R8/R9).
    ...
    return simulate_path_details(ordered_markets, expected, cap_rules=cap_rules)
...
def solve_hotelling_path(
    ...
    cap_rules : Sequence[CapRule]
        Rules for the COMPETITIVE FALLBACK only (bracket failure) — injected
        by the engine-bound entry point (``default_cap_rules(m0,
        "hotelling")``: per-year-gated MSR iff ``m0.msr_enabled``, never a
        CCR; F2/R8/R9). The primary Hotelling path applies no rules.

    Returns
    -------
    list[dict]
        Same structure as ``core.ledger.simulate_path_details`` — one dict
        per year.
    """
    ...
    if em_low <= total_budget or em_high >= total_budget:
        ...
        return _competitive_fallback(ordered_markets, cap_rules)
    ...
    return best_details or _competitive_fallback(ordered_markets, cap_rules)
