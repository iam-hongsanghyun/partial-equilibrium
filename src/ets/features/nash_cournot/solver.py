...
Moved from ``solvers/nash.py`` in the hotelling/nash feature order (v1 O11 /
v2 O15, ``docs/feature-modules-plan.md``), with the MSR state INJECTED as a
duck-typed object (plan §2 "injected duck-typed msr state; F2 preserved
bit-for-bit"): the engine-bound entry point
(``ets.engine.wiring.solve_nash_path``) constructs ``MSRState()`` iff the
first market enables the MSR; the APPLICATION block stays inline below,
UNGATED (``msr_start_year`` ignored, R16) with its own literal fallbacks and
no CCR (F2, R9). ``ets/solvers/nash.py`` remains as a re-export shim.
"""
...
def solve_nash_path(
    ...
    msr_state : Any | None
        ...
        entry point as ``MSRState()`` iff the first market enables the MSR
        (the retired inline construction). ``None`` applies no MSR. The
        application block below is F2-frozen: ungated (R16), literal
        fallbacks, no CCR (R9).

    Returns
    -------
    list[dict]
        Same structure as ``core.ledger.simulate_path_details``.
    """
