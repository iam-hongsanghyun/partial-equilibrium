...
Moved from ``solvers/transmission.py`` in the transmission feature order
(v1 O12 / v2 O16, ``docs/feature-modules-plan.md``), with the component-path
solvers INJECTED (plan §2 "component solvers injected; F3 stays internal"):
the engine-bound entry point (``ets.engine.wiring.solve_transmission_path``)
binds the RULE-CARRYING competitive binding as ``static_solver`` and the
Hotelling binding as ``hotelling_solver`` (economist item 5a) — MSR/CCR
apply inside the competitive component exactly as before. The
strip-floors → blend → clip order stays ONE internal function (F3 — never
split). ``ets/solvers/transmission.py`` remains as a re-export shim.
"""
...
def _evaluate_at_prices(
    ...
    Returns:
        Path details in the same structure as
...
def solve_transmission_path(
    ...
    static_solver: Callable[[list[CarbonMarket]], list[dict]] | None = None,
    hotelling_solver: Callable[..., list[dict]] | None = None,
    **hotelling_kwargs: Any,
    ...
    Args:
        ...
        risk_premium: Policy risk premium ρ added to r in the Hotelling
            component [1/yr].
        static_solver: Component-path solver for the static (competitive)
            price path, injected by the engine-bound entry point as the
            RULE-CARRYING competitive binding (economist item 5a — MSR/CCR
            apply inside the competitive component only). Called on the
            floor-stripped markets.
        hotelling_solver: Component-path solver for the inter-temporal
            price path, injected as the engine's Hotelling binding. Called
            on the floor-stripped markets with the rate and bisection
            keywords.
        **hotelling_kwargs: Extra keyword arguments forwarded to
            ``hotelling_solver`` (bisection iterations, tolerances).

    Returns:
        Path details in the same structure as
        ``core.ledger.simulate_path_details``, priced at the delivered path,
        with per-year component diagnostics (``static_component_price``,
        ``hotelling_component_price``, ``reserve_floor_price``,
        ``transmission_lambda``).
    ...
    competitive_path = static_solver(_strip_floors(ordered_markets))
    hotelling_path = hotelling_solver(
